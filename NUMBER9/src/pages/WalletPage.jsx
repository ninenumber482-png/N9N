...
export default function WalletPage() {
  ...
  return (
    <PageShell
      ...
      actions={
        <div className="hidden text-right sm:block">
          ...
          <p className="font-mono text-2xl font-black text-yellow-400">
            {fmt(balanceMain)} {t('common.points')}
          </p>
        </div>
      }>
      {/* TABS */}
      ...
      {tab === 'turnover' && (
        <TurnoverTab auth={auth} _rtTick={_rtTick} aliveRef={aliveRef} t={t} setToast={setToast} />
      )}
...
function DepositTab({
  ...
  const fallbackLockMs =
    lastDepositAt && nowTick - lastDepositAt < DEPOSIT_LOCK_MS ? DEPOSIT_LOCK_MS - (nowTick - lastDepositAt) : 0;
  const lockRemaining = useMemo(
    () => getMaxDepositLockRemaining(userTxs, nowTick, fallbackLockMs),
    [userTxs, nowTick, fallbackLockMs],
  );
  const inFifteenMinLock = lockRemaining > 0;
  const awaitingAdmin = useMemo(() => getDepositsAwaitingAdmin(userTxs, nowTick), [userTxs, nowTick]);
  const hasPendingDeposit = pendingDeposits.length > 0;
  const formBlocked = inFifteenMinLock || hasPendingDeposit;
  ...
  const handleProof = (e) => {
    ...
    r.onerror = () => {
      setProofName('');
      setToast({ type: 'err', text: t('common.file_read_error') });
    };
    ...
    try {
      ...
      setTimeout(() => {
        if (aliveRef.current)
          fetchUserTransactions(auth.id)
            .then((txs) => {
              if (aliveRef.current) setUserTxs(txs || []);
            })
            .catch(() => {});
      }, 500);
    } catch (err) {
      ...
      setToast({
        type: m === 'Request timeout' ? 'warn' : 'err',
        text: m === 'Request timeout' ? t('common.request_timeout') : m || t('common.network_error'),
      });
    }
  };
    ...
    <div className="space-y-6">
      {inFifteenMinLock && (
        <div className="rounded-xl border border-yellow-400/30 bg-yellow-400/10 p-4">
          ...
          <p className="mt-2 text-xs leading-relaxed text-yellow-400/80">{t('deposit.locked_desc')}</p>
          <div className="mt-2 h-1 overflow-hidden rounded-full bg-[#1f2128]">
            <div
              className="h-full bg-yellow-400"
              style={{ width: `${Math.round((lockRemaining / DEPOSIT_LOCK_MS) * 100)}%` }}
            />
          </div>
        </div>
      )}
      {!inFifteenMinLock && awaitingAdmin.length > 0 && (
        <div className="rounded-xl border border-orange-400/30 bg-orange-400/10 p-4">
          <p className="text-sm font-semibold text-orange-400">{t('deposit.expired')}</p>
          <ul className="mt-2 space-y-1">
            {awaitingAdmin.map((tx) => (
              <li key={tx.id} className="text-xs text-orange-400/80">
                {t('deposit.expired_ref', { amount: fmt(tx.amount), ref: tx.referenceCode || '—' })}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className={formBlocked ? 'pointer-events-none select-none opacity-45' : ''}>
        ...
        <section className="rounded-xl border border-[#1f2128] bg-[#0c0e14]">
          ...
          <div className="p-4">
            <label
              className={`block cursor-pointer rounded-lg border-2 border-dashed p-5 sm:p-8 text-center transition ${proof ? 'border-emerald-500 bg-emerald-500/10' : 'border-[#1f2128] hover:border-zinc-500'}`}>
              <input type="file" accept="image/*" className="hidden" onChange={handleProof} />
              {proof ? (
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-emerald-400">{t('common.file_selected')}</p>
                  <p className="text-xs text-emerald-400">{proofName}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-white">{t('common.click_to_upload')}</p>
                  <p className="text-xs text-zinc-500">{t('common.or_drag_drop')}</p>
                </div>
              )}
            </label>
            {proof && (
              <img
                src={proof}
                alt={t('deposit.proof')}
                className="mt-3 h-40 w-full rounded-lg border border-[#1f2128] object-cover"
              />
            )}
          </div>
        </section>
        ...
        <div className="rounded-xl border border-[#1f2128] bg-[#0c0e14] p-5">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between border-b border-[#1f2128] pb-2">
              <span className="text-zinc-400">{t('deposit.amount')}</span>
              <span className="font-bold text-white">
                {fmt(Number(amount) || 0)} {t('common.points')}
              </span>
            </div>
            {Number(amount) > 0 && (
              <div className="flex justify-between border-b border-[#1f2128] pb-2">
                <span className="text-zinc-400">{t('common.idr_value')}</span>
                <span className="font-bold text-yellow-400">{formatIDR(Number(amount))}</span>
              </div>
            )}
            <div className="flex justify-between border-b border-[#1f2128] pb-2">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${proof ? 'text-emerald-400' : 'text-zinc-500'}`}>
                {proof ? t('common.attached') : t('common.required')}
              </span>
            </div>
          </div>
          <button
            ...
            {loading
              ? t('common.processing')
              : inFifteenMinLock
                ? t('deposit.locked_timer_btn', { timer: fmtTimer(lockRemaining) })
                : hasPendingDeposit
                  ? t('deposit.pending_btn')
                  : t('deposit.confirm')}
          </button>
        </div>
      </div>
            ...
            {depositHistory.slice(0, 6).map((tx) => (
              <div key={tx.id} className="rounded-lg border border-[#1f2128] bg-[#0c0e14] px-4 py-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-xs font-bold ${tx.status === 'APPROVED' ? 'text-emerald-400' : tx.status === 'REJECTED' ? 'text-red-400' : 'text-yellow-400'}`}>
                    {tx.status}
                  </span>
                </div>
                <p className="text-sm font-semibold text-white">
                  {fmt(tx.amount)} {t('common.points')}{' '}
                  <span className="text-yellow-400/90 text-[11px] font-medium">({formatIDR(tx.amount)})</span>
                </p>
                <p className="text-xs text-zinc-500">
                  {tx.method} · {wibDate(tx.requestedAt)}
                </p>
              </div>
            ))}
          </div>
...
function WithdrawTab({ auth, balanceMain, _rtTick, aliveRef, t, setToast }) {
        ...
        if (bName || bNum || bHolder) {
          ...
          if (aliveRef.current && fb && (fb.bankName || fb.bankAccountNumber)) {
            setBank({
              ...
              bankAccountNumber: fb.bankAccountNumber || '',
              bankAccountName: fb.bankAccountName || '',
            });
          }
        }
      })
      .catch(() => {});
    const p2 = fetchTurnoverSummary(auth.id)
      .then((r) => {
        if (aliveRef.current) setTurnoverData(r);
      })
      .catch(() => {});
  ...
  const submit = async () => {
    ...
    if (hasActiveTurnover)
      return setToast({ type: 'err', text: t('withdraw.turnover_required', { amount: fmt(remainingTurnover) }) });
    setLoading(true);
    try {
      const r = await withTimeout(
        requestWithdraw({
          ...
          userUuid: auth.id,
          amount: amt,
          method: sel?.label || method,
          bankName: showBank ? finalBankName : '',
          bankAccountNumber: showBank ? finalBankAccNum : '',
          bankAccountName: showBank ? finalBankAccName : '',
        }),
        10000,
      );
      setLoading(false);
      ...
      if (useEdit && showBank && finalBankName && finalBankAccNum && finalBankAccName) {
        updateUserBank(auth.id, finalBankName, finalBankAccNum, finalBankAccName)
          .then(() =>
            setBank({ bankName: finalBankName, bankAccountNumber: finalBankAccNum, bankAccountName: finalBankAccName }),
          )
          .catch(() => {});
  ...
  return (
    <div className="space-y-6">
      <div
        className={`rounded-xl border p-4 ${hasActiveTurnover ? 'border-yellow-400/30 bg-yellow-400/10' : 'border-emerald-500/20 bg-emerald-500/5'}`}>
        <div className="flex items-center justify-between">
          <p className={`text-xs font-bold ${hasActiveTurnover ? 'text-yellow-400' : 'text-emerald-400'}`}>
            {t('withdraw.turnover_title')}
          </p>
        ...
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-[#1f2128]">
          <div
            className={`h-full transition-all duration-700 ${hasActiveTurnover ? 'bg-yellow-400' : 'bg-emerald-400'}`}
            style={{
              width: `${turnoverData.required > 0 ? Math.min(100, Math.round((turnoverData.achieved / turnoverData.required) * 100)) : 100}%`,
            }}
          />
        </div>
        <p className={`mt-1.5 text-xs ${hasActiveTurnover ? 'text-yellow-300' : 'text-emerald-400'}`}>
          {hasActiveTurnover
            ? t('withdraw.turnover_remaining', { amount: fmt(remainingTurnover) })
            : '✓ ' + t('turnover.unlocked')}
        </p>
      </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-zinc-500">
                      {t('withdraw.min_note', { note: m.note, min: m.min })}
                    </p>
                  </div>
                  {active && <Icon.Check size={18} className="text-emerald-400" />}
                </div>
            ...
            {!dataLoading && hasBankData && (
              <button
                ...
                onClick={() => {
                  ...
                  if (!isEditingBank) setEditBank({ name: bankName, number: bankAccNum, holder: bankAccName });
                }}
              ...
              <>
                <div>
                  <label className="mb-1 block text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                    {t('withdraw.bank_name')}
                  </label>
                  <input
                    value={useEdit ? editBank.name : bankName}
                    onChange={(e) => setEditBank((p) => ({ ...p, name: e.target.value }))}
                    readOnly={!useEdit}
                    className="w-full rounded-lg border border-[#1f2128] bg-[#13151c] px-3 py-2 text-base sm:text-sm text-white outline-none focus:border-yellow-400/50 read-only:opacity-70"
                    placeholder="e.g. BCA"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={useEdit ? editBank.number : bankAccNum}
                    onChange={(e) => setEditBank((p) => ({ ...p, number: e.target.value }))}
                    readOnly={!useEdit}
                    className="w-full rounded-lg border border-[#1f2128] bg-[#13151c] px-3 py-2 text-base sm:text-sm text-white outline-none focus:border-yellow-400/50 read-only:opacity-70"
                ...
                <div>
                  <label className="mb-1 block text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                    {t('withdraw.account_holder')}
                  </label>
                  <input
                    value={useEdit ? editBank.holder : bankAccName}
                    onChange={(e) => setEditBank((p) => ({ ...p, holder: e.target.value }))}
                    readOnly={!useEdit}
                    className="w-full rounded-lg border border-[#1f2128] bg-[#13151c] px-3 py-2 text-base sm:text-sm text-white outline-none focus:border-yellow-400/50 read-only:opacity-70"
      ...
      <div className="rounded-xl border border-[#1f2128] bg-[#0c0e14] p-5">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between border-b border-[#1f2128] pb-2">
            <span className="text-zinc-400">{t('withdraw.amount')}</span>
            <span className="font-bold text-white">
              {fmt(Number(amount) || 0)} {t('common.points')}
            </span>
          </div>
          ...
          <div className="flex justify-between border-b border-[#1f2128] pb-2">
            <span className="text-zinc-400">{t('withdraw.method')}</span>
            <span className="font-semibold text-white">{sel?.label || method}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-zinc-400">{t('withdraw.fee')}</span>
            <span className="font-semibold text-white">
              {(sel?.note?.split('·') ?? [])[1]?.trim?.() || t('withdraw.fee_free')}
            </span>
          </div>
        </div>
        <button
          type="button"
          onClick={() => setShowConfirm(true)}
          disabled={loading || !amount || Number(amount) <= 0 || hasActiveTurnover || (showBank && !hasFinalBank)}
          className="mt-4 w-full rounded-lg bg-yellow-400 py-3.5 text-sm font-extrabold text-black transition hover:bg-yellow-300 disabled:cursor-not-allowed disabled:opacity-50">
          {loading
            ? t('common.processing')
            : hasActiveTurnover
              ? t('withdraw.locked_required', { amount: fmt(remainingTurnover) })
              : !hasFinalBank && showBank
                ? t('withdraw.add_bank')
                : t('withdraw.request')}
        </button>
      </div>
...
function TurnoverTab({ auth, _rtTick, aliveRef, t, setToast }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'deposit_locks', filter: `user_id=eq.${auth.id}` },
        () => {
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-[#1f2128] bg-[#0c0e14]">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-zinc-500">
                {fmt(data.achieved)} / {fmt(data.required)}
              </span>
              <button
                onClick={refetch}
                disabled={refreshing}
        ...
        <div className="px-3 py-3 lg:px-4 lg:py-4">
          {data.isUnlocked ? (
            <>
              <p className="text-2xl font-black leading-tight text-yellow-400 lg:text-3xl">
                ✓ {t('turnover.unlocked')}
              </p>
              <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                {t('turnover.unlocked_sub')}
              </p>
            </>
          ) : (
            <>
              <p className="text-3xl font-black leading-none tabular-nums text-white lg:text-5xl">
                {data.pct}
                <span className="text-2xl text-yellow-400">{t('turnover.percent')}</span>
              </p>
              <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                {t('turnover.to_unlock')}
              </p>
            </>
          )}
          <div className="mt-3 h-2.5 overflow-hidden rounded-lg bg-[#1f2128]">
            <div
              ...
              style={{ width: `${data.isUnlocked ? 100 : data.pct}%` }}
            />
          </div>
          <div className="mt-3 grid grid-cols-2 gap-1.5 sm:grid-cols-4">
            {[
              [t('turnover.deposit'), fmt(data.totalDeposited)],
              [t('turnover.required'), fmt(data.required)],
              [t('turnover.done'), fmt(data.achieved), true],
              [t('turnover.left'), fmt(data.remaining)],
            ].map(([l, v, a]) => (
              <div key={l} className="rounded-lg border border-[#1f2128] bg-[#13151c] px-1.5 py-1.5 lg:px-2">
                ...
                <p
                  className={`mt-0.5 text-[12px] font-extrabold tabular-nums lg:text-sm ${a ? 'text-yellow-400' : 'text-white'}`}>
                  {v} {t('common.points')}
                </p>
              </div>
            ))}
          ...
          {!loading &&
            data.locks?.map((l, i) => (
              <div key={i} className="px-2.5 py-2 lg:px-3">
                <div className="flex items-center gap-2">
                  <p className="flex-1 text-[12px] font-bold text-white">
                    {t('turnover.deposit')} {fmt(l.amount)} P × 1×
                  </p>
                  {l.done ? (
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">
                      {fmt(l.applied)} / {fmt(l.required)} · {l.pct}%
                    </span>
                  )}
                </div>
                <div className="mt-1 h-1 overflow-hidden rounded-full bg-[#1f2128]">
                  <div
                    className={`h-full transition-all duration-700 ${l.done ? 'bg-yellow-400' : 'bg-zinc-500'}`}
                    style={{ width: `${l.pct}%` }}
                  />
                </div>
              </div>
            ...
            return (
              <div key={i}>
                <button
                  ...
                  <span className={`shrink-0 transition ${open ? 'rotate-180 text-yellow-400' : 'text-zinc-500'}`}>
      ...
      <div className="flex gap-2">
        <Link
          to={`/c/${auth?.id}/king`}
          className="flex-1 rounded-lg bg-yellow-400 py-2 text-center text-[11px] font-bold text-black hover:bg-yellow-300">
          {t('turnover.trade_marketplace')}
        </Link>
        <Link
          to={`/c/${auth?.id}/trading`}
          className="flex-1 rounded-lg border border-[#1f2128] py-2 text-center text-[11px] font-bold text-zinc-300 hover:text-white">
          {t('turnover.trade_now')}
        </Link>
      </div>
    </div>
  );
