# Security Center Full Console Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Turn the read-only Security Center into an actionable console — resolve alerts, clear/unlock failed logins, real usernames, KPI strip, severity/open filters, alert detail dialog, CSV export, auto-refresh polling — with **zero DB schema/RPC/migration/edge change**.

**Architecture:** All writes go through existing `admin-proxy` (service_role) via `admin.service` helpers (`updateRow` PATCH, generic `proxy` DELETE/PATCH). `security_alerts.resolved_at` and `failed_logins` rows already exist; both tables are in the proxy's `ALLOWED_PREFIXES`. A private `invalidate(fragment)` busts the 5 s GET cache after each mutation (critical — else resolved/cleared rows reappear). Realtime = 25 s polling (no Supabase channel → no publication change).

**Tech Stack:** Angular 22 (zoneless, OnPush, `inject()`), PrimeNG (`p-select`, `p-dialog`, `p-confirmdialog`, `p-paginator`), Tailwind 4. No test runner used for inline-template pages — `npm run build` is the gate (codebase convention).

**Verified facts:** `cache = new Map` (so `[...this.cache.keys()]` works); btn variants = `n9-btn-primary|n9-btn-danger|n9-btn-ghost` (NO `secondary`); `.dashboard-dialog.p-dialog` is the global branded modal class; proxy forwards any method (path-allowlist only) and auto-inserts an `audit_log` row per call; lockout = count of `failed_logins` (username+ip) within 900 000 ms.

---

### Task 1: admin.service — security actions + cache invalidation + username map

**Files:**
- Modify: `src/app/core/services/admin.service.ts` (add methods near the other AUDIT/security getters, ~line 654)

- [ ] **Step 1: Add the methods** (after `getFailedLogins`, before the GAME SESSIONS section)
...
git commit -m "✨ admin.service: actions Security Center (resolve alert, clear failed logins, user map) + cache bust"
```

---

### Task 2: Security Center component — full rewrite (actions, filters, KPI, detail dialog, CSV, polling)

**Files:**
- Modify (overwrite): `src/app/modules/dashboard/pages/security-center/security-center.component.ts`

- [ ] **Step 1: Overwrite the file with this complete content**
              ...
              <tbody>
                @for (l of displayItems; track l.id) {
                  <tr class="border-border hover:bg-accent/30 border-b">
                    ...
                    <td class="text-muted-foreground max-sm:px-1.5 max-sm:py-1.5 sm:px-5 sm:py-3.5 font-mono max-sm:hidden">
                      {{ l.ip_address || '-' }}
                    </td>
                  </tr>
                } @empty {
        ...
        @if (detailAlert) {
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              <span class="text-muted-foreground">Time</span><span>{{ detailAlert.created_at | wibDate: 'short' }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-muted-foreground">Type</span><app-status-badge [value]="detailAlert.alert_type" severity="warn" />
            </div>
            <div class="flex items-center justify-between">
            ...
            <div class="flex justify-between">
              <span class="text-muted-foreground">IP</span><span class="font-mono">{{ detailAlert.ip_address || '-' }}</span>
            </div>
            <div>
            ...
            @if (detailAlert.details) {
              <div>
                ...
                <pre class="bg-background border-border mt-1 overflow-x-auto rounded border p-2 text-xs">{{ detailAlert.details | json }}</pre>
              </div>
            }
            @if (!detailAlert.resolved_at) {
...
Run: `npm run build`
Expected: build succeeds. If a `status="secondary"` value isn't supported by `app-status-badge`, the template still compiles (it's a string input) — confirm via build, not types.

- [ ] **Step 3: Format**

Run: `npm run format`
Expected: Prettier rewrites whitespace/Tailwind order; no errors.

- [ ] **Step 4: Commit**

```bash
git add src/app/modules/dashboard/pages/security-center/security-center.component.ts
git commit -m "✨ security-center: console actionable (resolve, clear/unlock, KPI, filter, detail, CSV, auto-refresh)"
```
...
Run: `npm run build`
Expected: `Application bundle generation complete`. Output in `dist/number9systemd/browser/`.

- [ ] **Step 2: Deploy admin to Cloudflare Pages**

Run: `npx wrangler pages deploy dist/number9systemd/browser --project-name number9-admin --branch main --commit-dirty=true`
Expected: a deploy URL + success. (Confirm with user before running — live deploy.)

- [ ] **Step 3: Smoke check**

Run: `curl -s -o /dev/null -w "%{http_code}" https://admin.mynumber9.uk`
Expected: `200`.
...
**Spec coverage:** resolve alert (Task1 `resolveAlert` + Task2 confirmResolve/detail) ✓; resolve-all (`resolveAllOpenAlerts`/confirmResolveAll) ✓; clear/unlock failed logins (`clearFailedLogins`/confirmClearUser) ✓; clear-old (`clearOldFailedLogins`/confirmClearOld, 15 m cutoff) ✓; username map (alerts+audit via `userName`) ✓; severity + open-only filters ✓; KPI strip (open/critical/failed15/audit) ✓; alert detail dialog + JSON details ✓; CSV export per tab ✓; polling 25 s with guards + OnDestroy ✓; **cache invalidation** in every mutation ✓; no DB/RPC/edge change ✓.

**Placeholder scan:** none — every step has full code/commands.

**Type consistency:** `resolveAlert(id)`, `clearFailedLogins(username)`, `clearOldFailedLogins(cutoffISO)`, `getUserNameMap()`, `invalidate(fragment)` names match between Task 1 and Task 2 call sites. `SecurityItem` gains `resolved_at`/`details`. Buttons use existing `n9-btn-ghost|primary|danger` only. `status="secondary"` is passed as a plain string to `app-status-badge` (already used for "NOT SET" in the 2FA tab — safe).

**Risk note:** `app-status-badge value="RESOLVED" severity="secondary"` reuses the existing "secondary" severity already used in the 2FA tab, so it's known-good. Polling reuses cached getters at 25 s > 5 s TTL so it always fetches fresh.
