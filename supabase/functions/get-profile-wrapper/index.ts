...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Build headers with user's session token
    ...
    if (userToken) {
      rpcHeaders['x-user-token'] = userToken;
    }

    // Call get_my_full_profile RPC function
    const response = await fetch(
      `${supabaseUrl}/rest/v1/rpc/get_my_full_profile`,
      {
        method: 'POST',
        headers: rpcHeaders,
        body: JSON.stringify({}),
