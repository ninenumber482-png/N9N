...
const corsHeaders = (origin = "") => {
  const allow = ALLOWED_ORIGINS.includes(origin) || origin.endsWith('.number9-app.pages.dev') || origin.endsWith('.number9-admin.pages.dev') ? origin : ALLOWED_ORIGINS[0];
  return {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof dataUrl !== "string" || !dataUrl.startsWith("data:")) return json({ error: "Invalid file payload" }, 400);

    const match = dataUrl.match(/^data:([^;]+);base64,(.+)$/);
    if (!match) return json({ error: "Only base64 data URLs are supported" }, 400);
    const mime = match[1];
    const ext = ALLOWED_MIME[mime];
    if (!ext) return json({ error: `Unsupported file type: ${mime}` }, 400);

    const bytes = Uint8Array.from(atob(match[2]), (c) => c.charCodeAt(0));
    if (bytes.byteLength > MAX_BYTES) return json({ error: `File exceeds ${MAX_BYTES / 1024 / 1024}MB limit` }, 400);
