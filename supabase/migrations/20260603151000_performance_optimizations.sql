...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_rate_limit_counters_cleanup
  ON user_rate_limit_counters(window_start)
...
-- =============================================================================
-- 5. OPTIMIZE: engine_status view — ganti scalar subqueries dengan CTE
--    Mengurangi multiple table scans menjadi single pass
-- =============================================================================
...
latest_plan AS (
  SELECT MAX(updated_at) as last_plan_generated FROM king_planned
),
latest_metrics AS (
  SELECT
    MAX(recorded_at) FILTER (WHERE metric_name = 'king_engine_result_age_sec') as last_watchdog,
    MAX(metric_value) FILTER (WHERE metric_name = 'king_engine_result_age_sec') as result_age_sec
  FROM metrics
...
SELECT
  lr.last_result,
  lp.last_plan_generated,
  lm.last_watchdog,
  lm.result_age_sec,
  CASE
    WHEN lr.last_result IS NULL THEN 'NO_RESULTS'
    WHEN EXTRACT(EPOCH FROM (NOW() - lr.last_result)) > 420 THEN 'STALLED'
    ELSE 'RUNNING'
  END as engine_status
FROM latest_result lr
CROSS JOIN latest_plan lp
